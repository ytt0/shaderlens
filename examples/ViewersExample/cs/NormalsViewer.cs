...
public class NormalsViewer
{
    const float GridVisibleScale = 6.0f;
    const float NormalsVisibleScale = 3.0f;

    const float MaskContrastThreshold = 0.5f;

    const float ArrowThickness = 1.0f;
    const float ArrowSize = 0.9f;
    ...
    float sdNormalArrow(vec2 pos, float_ angle)
    {
