...
class ColorAdjustmentViewer
{
    //@uniform-group: color-adjustment-viewer
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly float_ imageBrightness = 0.0;
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly float_ imageContrast = 0.0;
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly float_ imageGamma = 0.0;

    //@uniform-group: color-adjustment-channels, parent: color-adjustment-viewer
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly vec3 channelBrightness = vec3(0.0);
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly vec3 channelContrast = vec3(0.0);
    //@uniform, min: -1.0, max: 1.0, step: 0.01
    readonly vec3 channelGamma = vec3(0.0);

    float_ adjustChannel(float_ channel, float_ brightness, float_ contrast, float_ gamma)
    {
