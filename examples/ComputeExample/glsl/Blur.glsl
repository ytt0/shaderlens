...
vec4 sample_local_cache(float offset)
{
...
vec4 sample_temp_cache(float offset)
{
    int index = int(floor(offset));
...
void main()
{
    ...
    const float[] kernel = float[](0.204164, 0.180174, 0.123832, 0.066282, 0.027631);

    float[] read_offset = float[](0.0, 0.25 * blurSize, 0.5 * blurSize, 0.75 * blurSize, blurSize);
